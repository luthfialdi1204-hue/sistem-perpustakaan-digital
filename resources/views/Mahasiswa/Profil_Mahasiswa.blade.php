  ...
  <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md">
    <div class="bg-gradient-to-br from-brand via-brand-light to-teal-600 relative px-6 py-8 md:px-8">
      ...
      <div class="relative flex flex-col items-center gap-4 sm:flex-row sm:items-end sm:gap-6">
        <div class="relative">
          <form id="formFotoProfil" method="POST" action="{{ route('mahasiswa.profil.foto') }}" enctype="multipart/form-data" class="group relative flex h-24 w-24 shrink-0 cursor-pointer items-center justify-center rounded-2xl border-4 border-white/30 bg-white/15 shadow-lg backdrop-blur-sm transition hover:border-white/60">
            @csrf
          ...
          @if($user->foto_profil)
            <form method="POST" action="{{ route('mahasiswa.profil.foto.hapus') }}" class="absolute -bottom-2 -right-2">
              @csrf
              @method('DELETE')
...
<script>
  ...
  document.getElementById('btnCropUpload').addEventListener('click', function () {
    ...
    cropper.getCroppedCanvas({
      ...
      fetch('{{ route("mahasiswa.profil.foto") }}', {
        method: 'POST',
        body: formData,
        headers: {
